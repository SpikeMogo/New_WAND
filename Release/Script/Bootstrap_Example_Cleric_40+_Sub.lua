...
------
maple.StopAtLevel = 70              -- stop at level = x
...
store.StoreMap=103000002              
store.NPCLocation = {-382, 40}          --stand close the npc, make sure you can open the NPC chat by key-pressing
store.NPCChatKey = vk.VK_SPACE            
store.SellWhenEquipsMoreThan=25         
...
store.Potion =
   {
      ...
      AutoHpThreshold=200,      
      AutoMpThreshold=1000,          
      BuyPotionList=
      {
         HP={ID=0000000,BuyNum=-1,  LowerLimit = -1},  
         MP={ID=2000006,BuyNum=800, LowerLimit = 20}, -- go the store if pot is below the LowerLimit
      }
   }
...
hunt.Attack =
    {
        ...
        Key2 = vk.VK_DELETE, --group attack  
        MobAttack = 1,        -- attack when mobs near you >= x
        MobFind = 1,          -- find mobs in a group of x
        Range = {
            IsFan = false,
            FanMinDistance = 60,
            Front = 200,      -- attack range in front: for spear, probably 120
            Back = 200,        -- attack range: back
